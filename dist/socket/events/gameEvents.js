...
function registerGameEvents(io, socket) {
    ...
    socket.on('client:vote_end_turn', () => {
        ...
        room.voteCount++;
        const eligible = Array.from(room.players.values()).filter((p) => p.status === 'alive' && !p.isArrested && !exports.blackjackSessions.has(p.codename));
...
function endTurn(io, room) {
    ...
    for (const [codename, session] of exports.blackjackSessions) {
        if (session.isSalvaguarda && session.roomId === room.id) {
            ...
            exports.blackjackSessions.delete(codename);
        }
    }
    ...
    (0, resolveAction_1.applyDownedDecay)(room, log);
    ...
    const classReveal = winner
        ? Object.fromEntries(Array.from(room.players.entries())
            ...
            .map(([codename, p]) => [codename, p.class]))
        : undefined;
...
function startNextRound(io, room) {
    ...
    for (const player of room.players.values()) {
        if (player.status === 'downed' && !player.salvaguardaPlayed && player.isConnected && !player.isArrested) {
            const session = (0, blackjack_1.createSalvaguardaSession)(player.codename, room.id);
            exports.blackjackSessions.set(player.codename, session);
