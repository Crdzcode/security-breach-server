import type { Room, QueuedAction, Player } from '../../types';
...
function getPriority(abilityName: string): number {
...
export function queueAction(
  ...
  abilityName: string,
  statType: import('../../types').StatType,
...
export function resolveAllActions(room: Room): string[] {
  const log: string[] = [];
...
function applyAbility(
  ...
  switch (action.abilityName) {
    ...
    case 'Sabotagem': {
      if (!target) break;
      const idx = room.pendingActions.findIndex(
        (a) => a.actorCodename === target.codename && a.abilityName === 'Labuta',
      );
      if (idx !== -1) {
    ...
    case 'Autópsia': {
      if (!target || target.status !== 'deceased') break;
...
export function applyPoisonDamage(room: Room, log: string[]): void {
...
export function applyDownedDecay(room: Room, log: string[]): void {
  for (const player of room.players.values()) {
